...
- *층별 기둥 설치율 제약* : 사용자는 0~1 사이의 값을 설정할 수 있다. **1(100%)**이면 하층부 기둥이 모두 설치가 완료될 때까지 상층부 기둥 설치를 진행하지 않으며, **0(0%)**이면 하층부 진행률과 관계없이 상층부 작업을 동시에 진행할 수 있다.
...
- *층별 기둥 설치율 제약 (Floor-level Column Installation Constraint)*
  
  임의의 층 `N` (N > 1)의 기둥 설치는, 바로 아래층인 `N-1`의 기둥 설치 진행률에 따라 제어됩니다. 예를 들어, "층 `N-1`의 기둥이 80% 이상 설치되기 전까지는 층 `N`의 기둥 설치를 시작할 수 없다"와 같은 규칙을 적용할 수 있습니다.
  
  사용자는 이 제약 조건의 임계값(%)을 조정하여, 하부층 안정화와 상부층 공사 진행 속도 간의 균형을 조절할 수 있습니다.
